...
def refreshHeroes():
    ...
    while(empty_scrolls_attempts >0):
        ...
        if buttonsClicked == 0:
            ...
            # Se a opção é diferente de todos, vamos rolar o scroll para buscar mais herois
...
def checkChests(newMap=False):
    if(mapIsActive()):
        ...
        wood_chest_closed = positions(images['wood_chest_closed'], threshold=0.5)
        wood_chest_hit = positions(images['wood_chest_hited'], threshold=0.5)
        steel_chest_closed = positions(images['steel_chest_closed'], threshold=0.5)
        #steel_chest_hit = positions(images['steel_chest_hited'], threshold=0.5)
        gold_chest_closed = positions(images['gold_chest_closed'], threshold=0.5)
        gold_chest_hit = positions(images['gold_chest_hited'], threshold=0.5)
        diamond_chest_closed = positions(images['diamond_chest_closed'], threshold=0.5)
        diamond_chest_hit = positions(images['diamond_chest_hited'], threshold=0.5)
        jail_closed = positions(images['jail_closed'], threshold=0.5)
        jail_hit = positions(images['jail_hited'], threshold=0.5)
        key_chest_closed = positions(images['key_chest_closed'], threshold=0.5)

        #TODO: colocar sprites do baú da chave com hit

        wood_qtd = len(wood_chest_hit)+len(wood_chest_closed)
        steel_qtd = len(steel_chest_closed)
        ...
        key_qtd = len(key_chest_closed)
        all_chests = wood_qtd+steel_qtd+gold_qtd+diamond_qtd+jail_qtd+key_qtd

        result = "🟫 Baús de madeira: {}\n🟪 Baús de ferro: {}\n🟨 Baús de ouro: {}\n🟦 Baús de diamante: {}\n🏛️ Jaulas: {}\n🗝️ Baú de chave: {}".format(wood_qtd, steel_qtd, gold_qtd, diamond_qtd, jail_qtd, key_qtd) 
        print(result)
...
def main():
    ...
    while True:
        ...
        if now - last["new_map"] > t['check_for_new_map_button']:
            ...
            if clickBtn(images['new-map'], threshold=0.5):
                sendMapImageToTelegram()
